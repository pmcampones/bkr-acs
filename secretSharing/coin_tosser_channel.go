...
type CoinTosserChannel struct {
	...
	outputChannels map[UUID]chan<- struct {
		bool
		error
	}
...
func NewCoinTosserChannel(node *network.Node, t uint, deal Deal, listenCode byte) *CoinTosserChannel {
	...
	channel := &CoinTosserChannel{
		...
		outputChannels: make(map[UUID]chan<- struct {
			bool
			error
		}),
...
func (c *CoinTosserChannel) TossCoin(seed []byte, outputChan chan<- struct {
	bool
	error
}) {
